#!/usr/bin/env python3
"""
3DDFA_V2 Multi-CSV Pitch Zero-Mean Plotter (No Pandas Required)
=============================================================

Visualize zero-mean pitch data from multiple CSV files in separate subplots.
Uses only matplotlib and numpy (no pandas dependency).

Usage:
    python3 plot_multiple_pitches.py [csv_file1] [csv_file2] ...
    python3 plot_multiple_pitches.py --name "custom_name" [csv_file1] [csv_file2] ...
"""
...
def plot_multiple_pitches(csv_files, save_dir, plot_name=None, supertitle=None):
    """Create separate subplots for each CSV file"""
    # Filter out empty CSV files
    valid_csv_files = []
    for csv_file in csv_files:
        data = load_csv_data(csv_file)
        if len(data['pitch']) > 0:  # Check if there's actual data
    ...
    for i, csv_file in enumerate(valid_csv_files):
        ...
        axes[i].set_title(f'{filename} - Pitch Detection, Zero-Mean (up is negative)')
        axes[i].grid(True, alpha=0.3)
        axes[i].axhline(y=0, color='k', linestyle='--', alpha=0.7)
        
        # Add statistics text
        stats_text = f'Range: {pitch_zero_mean.min():.1f}° to {pitch_zero_mean.max():.1f}°\nMean: {pitch_zero_mean.mean():.1f}°'
    ...
    if plot_name:
        save_path = os.path.join(save_dir, f'{plot_name}_pitch_zero_mean_plot.png')
    else:
        timestamp = int(time.time())
        save_path = os.path.join(save_dir, f'pitch_zero_mean_plot_{timestamp}.png')
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python plot_multiple_pitches.py [--name plot_name] [--supertitle supertitle] <csv1> <csv2> ...")
        sys.exit(1)
    ...
    plot_multiple_pitches(csv_files, save_dir, plot_name, supertitle)
