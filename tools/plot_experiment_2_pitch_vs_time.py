#!/usr/bin/env python3
"""
3DDFA_V2 Experiment 2 Pitch vs Time Plotter
===========================================

Plot pitch vs time for three videos (75°, 90°, 110°) in a single figure.
Focuses on pitch stability at angles closer to 90° as requested by professor.

Usage:
    python3 plot_experiment_2_pitch_vs_time.py
"""
...
def plot_experiment_2_pitch_vs_time():
    """Create pitch vs time plots for three videos (75°, 90°, 110°)"""
    
    # Define the CSV files for the three angles
    csv_files = [
        'examples/experiment/csv/75_pose_data.csv',
        'examples/experiment/csv/90_pose_data.csv', 
        'examples/experiment/csv/115_pose_data.csv'
    ]
    ...
    # Add supertitle
    plt.suptitle('Experiment 2: Pitch Stability at Angles Closer to 90°\nPitch vs Time Analysis', 
                fontsize=16, y=0.98)
    ...
    for i, (csv_file, angle_label) in enumerate(zip(csv_files, angle_labels)):
        ...
        stats_text = f'Std Dev: {pitch_std:.1f}° | Range: {pitch_range:.1f}° | Frames: {len(pitch)}'
    ...
    save_path = os.path.join(save_dir, f'experiment_2_pitch_vs_time_{timestamp}.png')
    
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"Experiment 2 pitch vs time plot saved to: {save_path}")
    plt.show()
