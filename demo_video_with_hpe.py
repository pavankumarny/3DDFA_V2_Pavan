# coding: utf-8
"""
3DDFA_V2 Video Demo Script WITH HPE DATA
=========================================

This script processes videos to perform 3D Dense Face Alignment (3DDFA) on faces.
EXACT COPY of demo_video.py with ONLY these additions:
1. HPE (Head Pose Estimation) data displayed on screen (bottom right)
2. Pitch, yaw, roll data stored in CSV file

NO CHANGES to the original algorithm logic.
"""
...
# ADDITION: Import for CSV writing and pose calculation
import csv
import cv2
...
def main(args):
    ...
    # ADDITION: Setup CSV file for HPE data
    csv_wfp = f'examples/results/videos/{fn.replace(suffix, "")}_{args.opt}_hpe.csv'
    csv_file = open(csv_wfp, 'w', newline='')
    csv_writer = csv.writer(csv_file)
    csv_writer.writerow(['frame', 'timestamp', 'pitch', 'yaw', 'roll'])  # Header
    ...
    for i, frame in tqdm(enumerate(reader)):
        ...
        # ADDITION: Extract HPE data using ORIGINAL algorithm (no changes)
        P, pose = calc_pose(param_lst[0])  # Use the exact same calc_pose as original
        yaw = pose[0]    # Original order: pose[0] = yaw
        pitch = pose[1]  # Original order: pose[1] = pitch  
        roll = pose[2]   # Original order: pose[2] = roll
        
        # ADDITION: Write to CSV file
        timestamp = i / fps
        csv_writer.writerow([i, timestamp, pitch, yaw, roll])
        ...
        # ADDITION: Draw HPE data on bottom right of frame
        h, w = res.shape[:2]
        ...
        overlay = res.copy()
        cv2.rectangle(overlay, (w-300, h-120), (w-10, h-10), (0, 0, 0), -1)
        cv2.addWeighted(overlay, 0.7, res, 0.3, 0, res)
        
        # Draw HPE text in bottom right
        y_start = h - 90
        cv2.putText(res, f"Pitch: {pitch:6.1f}", (w-290, y_start), font, font_scale, color, thickness)
        cv2.putText(res, f"Yaw:   {yaw:6.1f}", (w-290, y_start + 25), font, font_scale, color, thickness)
        cv2.putText(res, f"Roll:  {roll:6.1f}", (w-290, y_start + 50), font, font_scale, color, thickness)

        # Save the processed frame to output video
        writer.append_data(res[..., ::-1])  # Convert BGR back to RGB for video writer

    # Close video writer and CSV file
    writer.close()
    csv_file.close()
    print(f'Processed video saved to: {video_wfp}')
    print(f'HPE data saved to: {csv_wfp}')


if __name__ == '__main__':
    # Command line argument parser setup
    parser = argparse.ArgumentParser(description='3DDFA_V2 Video Processing Demo - Analyze faces in videos WITH HPE')
